...
describe('Валидация полей по схеме', () => {
  ...
  describe('createValidator', () => {
    ...
    test('должен вызвать функцию сообщения об ошибке', () => {
      ...
      const validateMock = jest.fn((value: Value<string>, config: string) => false);
    ...
    test('должен вызвать strictValidate, если передан validate', () => {
      const errorMessage = '';
      const messageFnMock = jest.fn((value: Value<string>, config: string) => errorMessage);
      const strictValidateMock = jest.fn((value: Value<string>, config: string) => false);
  ...
  describe('validators', () => {
    ...
    test('isGroupObjectAlreadyExist', () => {
      const mockGroupObjects = [
        { id: 'mockId1', name: 'mock name1', nodeList: [] },
        { id: 'mockId2', name: 'mock name2', nodeList: [] },
        { id: 'mockId3', name: 'mock name3', nodeList: [] },
      ] as TreeItem[];
      const validate = validators.isGroupObjectAlreadyExist(mockGroupObjects);
      expect(validate('mock name')).toBe(null);
      expect(validate('mock name1')).not.toBe(null);
      expect(validate('   mock name1   ')).not.toBe(null);
      expect(validate('  mock name2')).not.toBe(null);
      expect(validate('mock name3  ')).not.toBe(null);
      expect(validate('another mock name')).toBe(null);
    });
  });
});
